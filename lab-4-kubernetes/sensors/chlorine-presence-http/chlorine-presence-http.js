...
const deviceConnectionString = 'HostName=water-quality-iot-hub.azure-devices.net;DeviceId=chlorine-presence-http;SharedAccessKey=I66o5YCtz88mmgb/5CzUfIo4IdqmdArclE9qCMfApbA=';
let sendInterval;
...
function generateMessage () {
  const chlorinePresence = Math.random() * 100; // range: [0, 100]
  var timestamp = new Date();
  const data = JSON.stringify(
    {
      deviceId:    'chlorinePresenceDevice',
      value:       chlorinePresence,
      measureTime: getTimestamp(timestamp),
      protocol:    'http'
    }
  );
  const message = new Message(data);
  message.properties.add('chlorinePresenceDeviceAlert', (chlorinePresence > 10) ? 'true' : 'false');
  return message;
}
