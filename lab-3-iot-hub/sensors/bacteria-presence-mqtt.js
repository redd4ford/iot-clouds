...
const deviceConnectionString = 'HostName=water-quality-iot-hub.azure-devices.net;DeviceId=bacteria-presence-mqtt;SharedAccessKey=DiAPF/bBYT5LAlFXKnkzGFoVIhKmX+xcOfCq96BOoDo=';
let sendInterval;
...
function generateMessage () {
  const bacteriaPresence = Math.random() * 50; // range: [0, 50]
  var timestamp = new Date();
  const data = JSON.stringify(
    {
      deviceId:    'bacteriaPresenceDevice',
      value:       bacteriaPresence,
      measureTime: getTimestamp(timestamp),
      protocol:    'mqtt'
    }
  );
  const message = new Message(data);
  message.properties.add('bacteriaPresenceAlert', (bacteriaPresence > 5) ? 'true' : 'false');
  return message;
}
